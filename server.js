...
// OMNIVERSE - NO SQLite, Using JSON File Storage
// Works perfectly on Heroku!
// ======================================================
...
const YOUR_DOMAIN = process.env.DOMAIN || 'zass.website';
...
function loadData() {
    ...
    return { apps: [], nextPort: 3001, totalDeployments: 0 };
}
...
async function loadExistingApps() {
    ...
    console.log(`✅ Loaded ${deployedApps.size} apps from storage`);
}
...
class AIAssistant {
    ...
    async answerQuestion(question) {
        ...
        if (q.includes('how to deploy')) {
            return "To deploy: Paste your code, give it a name, and click Deploy. Your app will be live immediately!";
        }
        if (q.includes('github')) {
            return "Enter your GitHub repository URL (e.g., https://github.com/username/repo). Make sure it has a server.js file!";
        }
        return "I'm OmniAI! I can help you deploy apps, fix code issues, and answer questions.";
    }
}
...
class DeploymentEngine {
    getRealUrl(appName) {
        ...
        if (IS_PRODUCTION && process.env.HEROKU_APP_NAME) {
            return `https://${cleanName}.${YOUR_DOMAIN}`;
        }
        return `http://localhost:${appData.nextPort}`;
    }
...
app.get('/api/apps', (req, res) => {
    ...
    res.json({ apps, total: apps.length, totalDeployments: appData.totalDeployments });
});
...
<head>
    ...
    <title>OmniVerse - Deploy Your Apps</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .app-card a { color: #a8c0ff; text-decoration: none; }
...
<body>
    <div class="container">
        <div class="header">
            <h1>🚀 OmniVerse</h1>
            <p>Deploy any app - Get a live URL instantly!</p>
        </div>
    ...
    <script>
        ...
        async function deployCode() {
            ...
            if (result.success) {
                showAlert('deployResult', \`✅ <strong>SUCCESS!</strong><br>🔗 <a href="\${result.url}" target="_blank">\${result.url}</a>\`, 'success');
...
║     💾 Storage: JSON File (No SQLite needed!)            ║
║     🔄 Apps persist across restarts!                      ║
║                                                            ║
╚════════════════════════════════════════════════════════════╝
        `);
    });
